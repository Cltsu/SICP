    ...
    (let ((i (an-integer-starting-from 1)))
        (let ((j (an-integer-starting-from i)))
            (let ((k (an-integer-starting-from j)))
                ...
                (list i j k)))))
;4.37
;ben的方法会快些，4.35的方法为O(n^3),改方法为O(n^2logn)
;4.38
;不知道
;4.39
;只会影响时间
...
;4.40
;对每个人套一层let，amd之后立即用require询问是否distinct以及一些其他条件
;4.41
;4.42
...
;4.44
(define (eight-queen)
    (let ((queens (map (lambda (x) (cons x (an-integer-between 1 8)))
                       (list 1 2 3 4 5 6 7 8))))
        (define (tra pos) (+ (car pos) (cdr pos)))
        (define (tla pos) (- (car pos) (cdr pos)))
        (require (distinct? (map cdr queens)))
        (require (distinct? (map tra queens)))
        (require (distinct? (map tla queens)))
        queens))
